...
const func: DeployFunction = async ({ deployments, getNamedAccounts, getUnnamedAccounts, ethers }) => {
  ...
  const usdt = await ethers.getContractAt('MockERC20', stables[1], deployer);
  const usdc = await ethers.getContractAt('MockERC20', stables[0], deployer);
  const wavaxC = await ethers.getContractAt('MockERC20', wavax, deployer);
  ...
  // await usdt.approve(aggregator.address, ethers.constants.MaxInt256);
  // await usdc.approve(aggregator.address,  ethers.constants.MaxInt256);

  // await tokenReqt.approve(aggregator.address,  ethers.constants.MaxInt256);
  // await wavaxC.approve(aggregator.address,  ethers.constants.MaxInt256);
  ...
  await execute(
    'aggregator',
    { from: deployer, gasLimit: 8e6, value :ethers.BigNumber.from('1000000000000000') 
    },
    'multiSwapETHForExactTokens',
    [[wavax, stables[0]]],
    [1],
    ethers.BigNumber.from('1'), // amount out
    ethers.BigNumber.from('900000000000000000000000000'), //deadline
  );
  ...
  await execute(
    'aggregator',
    { from: deployer, gasLimit: 8e6, value :ethers.BigNumber.from('1000000000000') ,
   },
    'multiSwapETHForExactTokens',
    [[wavax, reqt, stables[1]], [ stables[1], stables[0]]],
    [1, 0],
    ethers.BigNumber.from('10'), // amount out
    ethers.BigNumber.from('900000000000000000000000000'), //deadline
  );
  console.log("--- 10 succssful ---")
  ...
  await execute(
    ...
    'multiSwapETHForExactTokens',
    [[ wavax, stables[0]], [stables[0], stables[1]]],
    [1, 0],
    ethers.BigNumber.from('10'), // amount out
    ethers.BigNumber.from('900000000000000000000000000'), //deadline
  );

  console.log("-- 10 (2) successful ---")

  await execute(
    'aggregator',
    { from: deployer, gasLimit: 8e6, value :ethers.BigNumber.from('1000000000000')  },
    'multiSwapETHForExactTokens',
    [[wavax, stables[0]], [stables[0], stables[1]], [stables[1], reqt]],
    [1, 0, 1],
    ethers.BigNumber.from('10'), // amount out
    ethers.BigNumber.from('900000000000000000000000000'), //deadline
  );
  ...
  await execute(
    ...
    { from: deployer, gasLimit: 8e6, value :ethers.BigNumber.from('1000000000000')  },
    'multiSwapETHForExactTokens',
    [[wavax, stables[0]], [stables[0], stables[1]], [stables[1], reqt, wavax, stables[0]], [stables[0], stables[1]]],
    [1, 0, 1, 0],
    ethers.BigNumber.from('10'), // amount out
    ethers.BigNumber.from('900000000000000000000000000'), //deadline
  );
